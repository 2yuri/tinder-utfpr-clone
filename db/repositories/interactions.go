...
func ShowLiked(ctx context.Context, userId string) ([]*domain.User, error) {
	...
	query := ` SELECT u.name, u.email, u.instagram, u.about, u.sex_preference,  u.find_distance, u.birth_date, 
		...
		where u.external_id = $1 AND i.liked = true`
...
func CreateInteraction(ctx context.Context, userId string, targetUserId string, like bool) (bool, error) {
	var has bool
	...
	_, err := db.Db.ExecContext(ctx, query, userId, targetUserId, like)
	if err != nil {
		return has, err
	}

	_ = db.Db.QueryRowContext(ctx, `SELECT liked FROM interactions 
            WHERE user_id = (select id from users where external_id = $1) 
            AND target_user_id = (select id from users where external_id = $2)`, targetUserId, userId).Scan(&has)
